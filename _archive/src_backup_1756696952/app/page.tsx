...
// 如果将来要用到再解注：import SolutionsGrid from "@/components/SolutionsGrid";
...
function Hero() {
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-gray-50 to-white">
      {/* Background：本地港口图 + 海洋系遮罩 */}
      <div className="absolute inset-0 -z-10">
        <Image
          src="/images/hero-port.jpg"
          alt="Container terminal overview"
          fill
          sizes="100vw"
          className="object-cover"
          priority
        />
        <div className="absolute inset-0 bg-gradient-to-b from-sky-50/80 via-cyan-50/80 to-white" />
      </div>
      ...
      <div className="mx-auto max-w-6xl px-5 py-16 md:py-20">
        <div className="max-w-3xl">
          ...
          <p className="mt-5 max-w-2xl text-xl text-slate-700">
            Standardized endpoints for <span className="font-medium">congestion</span>, <span className="font-medium">yard dwell</span>,
            <span className="font-medium"> berth wait &amp; ETA/ETB forecasts</span> — plus corridor snapshots, historical trends and alerts.
            Measurable coverage, p95 &lt;300&nbsp;ms, 30-day replay.
          </p>
          ...
          <p className="mt-4 text-lg text-gray-600">
            PortPulse provides reproducible data for trends, congestion, dwell, snapshots and alerts. Get running in 5
            minutes, integrate in 30.
          </p>
...
function DataKPI() {
  const items = [
    ["67+ ports", "coverage (today)"],
    ["<300ms p95", "typical API latency"],
    ["30-day replay", "CSV/JSON reproducible"],
    ["99.9% SLA", "status & probes"],
  ] as const;

  return (
    <section className="bg-white">
      <div className="mx-auto max-w-6xl px-4 md:px-6 lg:px-8 py-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {items.map(([k, v], i) => (
          <div key={i} className="rounded-xl border bg-slate-50 p-5">
...
function Solutions() {
  const cards = [
    ...
    ["Predictive ETAs & Alerts", "Actionable exceptions with reasons and thresholds."],
  ...
  return (
    <section className="bg-slate-50/60 border-y border-slate-200/70">
      <div className="mx-auto max-w-6xl px-4 md:px-6 lg:px-8 py-12 grid gap-6 md:grid-cols-2">
        {cards.map(([t, d], i) => (
          <div key={i} className="rounded-xl border bg-white p-6">
            <div className="text-lg font-semibold">{t}</div>
            <p className="mt-2 text-slate-600">{d}</p>
            <a href="/contact" className="mt-4 inline-block text-sm text-sky-700 underline underline-offset-4">
              Learn more
            </a>
          </div>
        ))}
...
const GALLERY = [
  ...
  { src: "https://images.unsplash.com/photo-1556403189-51f9b8c9850e?q=80&w=1600&auto=format&fit=crop", alt: "Rotterdam" },
...
/** -------------------- Trend widget -------------------- **/
type TrendMiniProps = { unlocode: string; days?: number; height?: number };
  ...
  return (
    <div className="rounded-xl border border-slate-200">
      ...
      <div className="px-3 pb-3">
        <pre className="whitespace-pre-wrap rounded-lg border border-slate-200 bg-slate-50 p-3 text-[11px] leading-relaxed">{`curl -sS -H "X-API-Key: ${DEMO_KEY || "dev_demo_123"}" \\
  "${API_BASE}/v1/ports/${unlocode}/trend?days=${days}" | jq .`}</pre>
      </div>
    </div>
  );
}
...
/** -------------------- ResourcesStrip -------------------- **/
type LinkItem = { href: string; label: string; ext?: boolean };
      ...
      <section className="container mx-auto px-4 pb-4">
        <div className="rounded-2xl border border-slate-200 p-6">
          <div className="mb-2 flex items-center justify-between">
            ...
            <a className="text-sm text-slate-500 underline underline-offset-4" href="https://docs.useportpulse.com/openapi.json" target="_blank" rel="noreferrer">OpenAPI</a>
          </div>
          <TrendMini unlocode="USLAX" days={7} height={224} />
          <div className="mt-4">
            <code className="block whitespace-pre-wrap rounded-lg border border-slate-200 bg-slate-50 p-3 text-xs">{`curl -sS -H "X-API-Key: ${DEMO_KEY}" \\
  "${API_BASE}/v1/ports/USLAX/trend?days=7" | jq .`}</code>
            <button
              type="button"
              onClick={() => navigator.clipboard.writeText(`curl -sS -H "X-API-Key: ${DEMO_KEY}" "${API_BASE}/v1/ports/USLAX/trend?days=7" | jq .`)}
