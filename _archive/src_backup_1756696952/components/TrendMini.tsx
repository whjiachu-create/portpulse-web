...
type Props = { unlocode: string; days?: number; height?: number };
...
export default function TrendMini({ unlocode, days = 7, height = 224 }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const j = (await res.json()) as { points?: { date: string; congestion_score?: number; vessels?: number }[] };
        const pts: Pt[] = (j.points ?? []).map((p) => ({
          ts: p.date,
          v: typeof p.congestion_score === "number" ? p.congestion_score :
             typeof p.vessels === "number" ? p.vessels : 0,
        }));
...
{`curl -sS -H "X-API-Key: ${DEMO_KEY || "dev_demo_123"}" "${API_BASE}/v1/ports/${unlocode}/trend?days=${days}" | jq .`}
        </pre>
      </div>
    </div>
  );
