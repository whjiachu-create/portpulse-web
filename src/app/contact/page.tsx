...
export default function ContactPage() {
  ...
  const [error, setError] = useState<string>(""); const [ok, setOk] = useState<boolean | null>(null);
  const json = `Contact request\n${new Date().toISOString()}\n${JSON.stringify(form, null, 2)}`;
  const mailto = `mailto:hello@useportpulse.com?subject=${encodeURIComponent("PortPulse contact")}&body=${encodeURIComponent(json)}`;
  useEffect(() => {
    const intent = params.get("intent") || ""; const port = params.get("port"); const country = params.get("country");
    if (intent === "port_request") setForm(s => ({ ...s, message: s.message || `Request new port: ${port ?? ""}${country ? `, ${country}` : ""}` }));
  }, [params]);
  async function onSubmit(e: React.FormEvent) {
    e.preventDefault(); setError(""); setOk(null);
    if (!isBusinessEmail(form.email)) { setError("请使用工作邮箱（非公共邮箱，如 gmail/outlook/yahoo 等）"); return; }
    try {
      const res = await fetch("/api/contact", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
      if (!res.ok) throw new Error(String(res.status)); setOk(true);
    } catch { setOk(false); window.location.href = mailto; }
  }
  return (
