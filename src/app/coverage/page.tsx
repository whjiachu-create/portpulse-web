...
export default function CoveragePage() {
  ...
  function dlCsv(rows: PortItem[]) {
    ...
    const body = rows.map(r => [r.port, r.country, r.region, r.status, r.metrics.join("/"), r.freshness, r.notes ?? ""]);
    const csv = [head, ...body].map(x => x.map(v => `"${String(v).replace(/"/g,'""')}"`).join(",")).join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8" });
    const a = document.createElement("a"); a.href = URL.createObjectURL(blob);
    a.download = "portpulse_coverage.csv"; a.click(); URL.revokeObjectURL(a.href);
  }
  ...
  return (
    <div className="container mx-auto px-4 py-12 md:py-16">
      ...
      <div className="mt-6 overflow-x-auto rounded-2xl border border-black/10 bg-white">
        <table className="w-full text-sm">
          <thead className="text-left bg-black/5">
            <tr>
              <Th>Port</Th><Th>Country</Th><Th>Region</Th><Th>Status</Th><Th>Metrics</Th><Th>Freshness SLO</Th><Th>Notes</Th><Th>Action</Th>
            </tr>
          </thead>
          <tbody>
            {filtered.length===0 && (<tr><td className="p-4 text-black/60" colSpan={8}>No ports found. Try a different filter.</td></tr>)}
            {filtered.map((p,i)=>(
      ...
      <p className="mt-3 text-black/60 text-sm">New ports follow the same schema and SLO. Typical onboarding 2–4 weeks.</p>
    </div>
  );
}
